...
public sealed partial class VpnViewModel : ObservableObject
{
    ...
    {
        if (AppServices.VpnProfileImporter is not { } importer)
        {
            await _dialogs.ShowMessageAsync("VPN", "Importing profiles is not supported on this platform.");
            return;
        }
        ...
        try
        {
            var profile = importer.Import(path, Profiles);
    ...
    private async Task RemoveProfileAsync()
    {
        ...
        AppServices.VpnProfileImporter?.DeleteConfig(profile);
