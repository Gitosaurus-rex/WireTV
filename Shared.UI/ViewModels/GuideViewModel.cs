...
/// <summary>
/// Which column of the guide the remote is driving, ordered left to right.
///
/// Days is a column rather than a pair of shortcut keys because a TV remote has
/// nothing spare to bind: walking left from the channel list is the only way to
/// reach it with a D-pad.
/// </summary>
public enum GuidePane
{
    Days,
    Channels,
    Programmes
...
public sealed partial class GuideViewModel : ObservableObject, IDisposable
{
    ...
    /// <summary>Which column the remote drives. See RemoteKey for why focus is not used.</summary>
    [ObservableProperty]
    [NotifyPropertyChangedFor(nameof(IsDayPaneActive), nameof(IsChannelPaneActive), nameof(IsProgrammePaneActive))]
    private GuidePane _pane = GuidePane.Channels;

    public bool IsDayPaneActive => Pane == GuidePane.Days;

    public bool IsChannelPaneActive => Pane == GuidePane.Channels;

    public bool IsProgrammePaneActive => Pane == GuidePane.Programmes;

    /// <summary>Moves the highlight within the active column.</summary>
    public void Move(int delta)
    {
        switch (Pane)
        {
            case GuidePane.Days:
                Step(Days, SelectedDay, delta, item => SelectedDay = item);
                break;

            case GuidePane.Channels:
                Step(Channels, SelectedChannel, delta, item => SelectedChannel = item);
                break;

            default:
                Step(Programmes, SelectedProgramme, delta, item => SelectedProgramme = item);
                break;
        }
    }

    /// <summary>Steps one column left or right, clamped at the ends.</summary>
    public void StepPane(int delta)
        => Pane = (GuidePane)Math.Clamp((int)Pane + delta, (int)GuidePane.Days, (int)GuidePane.Programmes);

    /// <summary>
    /// OK moves rightwards through the columns and only starts playback once the
    /// remote is actually on a programme or a channel.
    /// </summary>
    public bool Activate()
    {
        if (Pane == GuidePane.Days)
        {
    ...
    private static void Step<T>(IReadOnlyList<T> items, T? current, int delta, Action<T> select)
        where T : class
    {
        if (items.Count == 0)
            return;

        var index = current is null ? -1 : IndexOf(items, current);
        select(items[index < 0 ? 0 : Math.Clamp(index + delta, 0, items.Count - 1)]);
    }
    ...
    [RelayCommand]
    private void PreviousDay() => StepDay(-1);

    /// <summary>Clamped rather than wrapping: running off the end of the guide is not a useful move.</summary>
    private void StepDay(int delta)
    {
        if (SelectedDay is null)
            return;

        var index = Days.ToList().IndexOf(SelectedDay);

        if (index < 0)
            return;

        SelectedDay = Days[Math.Clamp(index + delta, 0, Days.Count - 1)];
    }
